...
export async function updateClienteAction(id: string, input: ClienteInput): Promise<ActionResult<void>> {
  try {
...
export async function deleteClienteAction(id: string): Promise<ActionResult<void>> {
  try {
    ...
    if (ordersCount > 0) {
      return { success: false, error: "No se puede eliminar el cliente porque tiene pedidos asociados" };
    }
