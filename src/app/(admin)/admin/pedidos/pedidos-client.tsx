...
import { FileText, ArrowLeft, ArrowRight, Search, Calendar, DollarSign, Eye } from 'lucide-react';
...
const ESTADOS = [
  { key: 'cotizado', label: 'Cotizado', color: 'border-slate-800 bg-slate-900/10 text-slate-400' },
  {
    key: 'pagado',
    label: 'Pagado',
    color: 'border-blue-500/30 bg-blue-500/5 text-blue-400 ring-1 ring-blue-500/10',
  },
  {
    key: 'en_produccion',
    label: 'En Producción',
    color: 'border-violet-500/30 bg-violet-500/5 text-violet-400 ring-1 ring-violet-500/10',
  },
  {
    key: 'entregado',
    label: 'Entregado',
    color: 'border-amber-500/30 bg-amber-500/5 text-amber-400 ring-1 ring-amber-500/10',
  },
  {
    key: 'completado',
    label: 'Completado',
    color: 'border-emerald-500/30 bg-emerald-500/5 text-emerald-400 ring-1 ring-emerald-500/10',
  },
];
...
export function PedidosClient({ initialPedidos }: PedidosClientProps) {
  ...
  const filteredPedidos = pedidos.filter((p) => {
    ...
    if (p.datosInvitacion) {
      try {
        datos =
          typeof p.datosInvitacion === 'string'
      ...
      <div className="grid gap-6 md:grid-cols-3 lg:grid-cols-5 items-start">
        {ESTADOS.map((col) => {
          ...
          return (
            <div
              key={col.key}
              className="flex flex-col gap-4 rounded-xl border border-slate-900/60 bg-slate-900/5 p-4 min-h-[500px]"
            >
              {/* Column Title Header */}
                          ...
                          <div className="flex items-center gap-1.5 text-3xs text-slate-400">
                            ...
                            <span>
                              {new Intl.DateTimeFormat('es-MX', {
                                ...
                                month: 'short',
                                year: '2-digit',
                              }).format(new Date(pedido.fechaEvento))}
                            </span>
                          </div>
                          <div className="flex items-center gap-1.5 text-3xs text-slate-400">
