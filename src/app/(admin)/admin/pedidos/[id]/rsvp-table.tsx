      ...
      <CardContent className="pt-6 space-y-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-slate-950/40 border border-slate-900 rounded-xl p-3.5 text-center">
            <p className="text-2xs uppercase tracking-wider text-slate-500 font-semibold mb-1">Confirmados (Pax)</p>
            <p className="text-xl font-bold text-violet-400">{paxConfirmados}</p>
            <p className="text-[10px] text-slate-600">personas en total</p>
          </div>
          <div className="bg-slate-950/40 border border-slate-900 rounded-xl p-3.5 text-center">
            <p className="text-2xs uppercase tracking-wider text-slate-500 font-semibold mb-1">Esperados</p>
            <p className="text-xl font-bold text-white">{invitadosEsperados}</p>
            <p className="text-[10px] text-slate-600">invitados estimados</p>
          </div>
          <div className="bg-slate-950/40 border border-slate-900 rounded-xl p-3.5 text-center">
            <p className="text-2xs uppercase tracking-wider text-slate-500 font-semibold mb-1">Asistirán</p>
            <p className="text-xl font-bold text-emerald-400">{asistenCount}</p>
            <p className="text-[10px] text-slate-600">respuestas positivas</p>
          </div>
          <div className="bg-slate-950/40 border border-slate-900 rounded-xl p-3.5 text-center">
            <p className="text-2xs uppercase tracking-wider text-slate-500 font-semibold mb-1">No Asistirán</p>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs font-semibold">
            ...
            <span className="text-violet-400">
              {paxConfirmados} de {invitadosEsperados} confirmados ({Math.round((paxConfirmados / invitadosEsperados) * 100) || 0}%)
            </span>
          </div>
          <div className="h-2 w-full bg-slate-950 rounded-full overflow-hidden border border-slate-900">
            <div 
              style={{ width: `${Math.min(100, Math.round((paxConfirmados / invitadosEsperados) * 100))}%` }}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-1.5 bg-slate-950/40 border border-slate-900 p-1 rounded-lg">
            ...
            <button
              ...
              className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors ${
                filterType === "asiste" ? "bg-emerald-600/10 text-emerald-400 border border-emerald-500/20" : "text-slate-400 hover:text-white"
              }`}
            >
              Asiste ({asistenCount})
            </button>
            <button
              ...
              className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors ${
                filterType === "no-asiste" ? "bg-rose-600/10 text-rose-400 border border-rose-500/20" : "text-slate-400 hover:text-white"
              }`}
            >
              ...
              <tbody className="divide-y divide-slate-900 bg-slate-950/20">
                {filteredRSVPs.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-900/40 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-center text-slate-100 font-mono">{r.asiste ? r.pax : 0}</td>
                    <td className="px-4 py-3.5 text-slate-400 font-mono text-xs">{r.telefono || "-"}</td>
                    <td className="px-4 py-3.5 text-slate-400 max-w-[200px] truncate" title={r.mensaje || ""}>
                    ...
                    <td className="px-4 py-3.5 text-right text-slate-500 text-xs">
                      {new Intl.DateTimeFormat("es-MX", { dateStyle: "short", timeStyle: "short" }).format(new Date(r.createdAt))}
                    </td>
                  </tr>
                ))}
              </tbody>
