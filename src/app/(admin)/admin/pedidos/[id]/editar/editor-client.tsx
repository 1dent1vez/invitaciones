  ...
  for (const field of templateConfig.fields) {
    if (field.required) {
      ...
      if (field.type === 'gallery') {
        ...
        base[field.key] = urlValidation.refine((val) => !!val, {
          message: `El campo "${field.label}" es requerido`,
        });
      } else if (field.key === 'edad' || field.key === 'numPases') {
        base[field.key] = z.preprocess(
          preprocessNumber,
          z.number({ required_error: `El campo "${field.label}" es requerido` })
        );
      } else {
      ...
      return (
        <div className="p-6 text-center text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-xl m-4 space-y-2">
          ...
          <p className="text-xs text-rose-300">
            Los datos ingresados causaron un error al renderizar la vista previa.
          </p>
...
function getAccordionGroup(field: { key: string }) {
  ...
  if (
    key === 'historiaEdad' ||
    key === 'historiaSeresQueridos' ||
    key === 'historiaRecuerdo' ||
    key.startsWith('historia')
  ) {
    return 'historia';
  }
  return 'otros';
...
export function EditorClient({ pedido: initialPedido, initialIsLoading }: EditorClientProps) {
  ...
  const [currentFormTab, setCurrentFormTab] = useState<'info' | 'fotos' | 'diseno' | 'rsvp'>(
    'info'
  );
  ...
  const dbDatos = useMemo(
    () => (pedido.datosInvitacion as unknown as InvitacionData) || {},
    [pedido.datosInvitacion]
  );
        ...
        const handleAutoSave = async () => {
          ...
          try {
            ...
            if (res.success && res.data) {
              setPedido((prev) => ({
                ...prev,
                datosInvitacion: res.data as unknown as Prisma.JsonValue,
              }));
              setIsDirty(false);
  ...
  const handleImageUpload = async (
    ...
    fieldKey: keyof EditorFormValues
  ) => {
    ...
    return (
      <div key={field.key} className="space-y-2">
              ...
              {PRESET_COLORS.map((color) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setValue(registerKey, color.value, {
                        shouldDirty: true,
                        shouldValidate: true,
                      })
                    }
            ...
            >
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setValue(registerKey, e.target.value, {
                      shouldDirty: true,
                      shouldValidate: true,
                    })
                  }
            ...
            {watchedData[registerKey] ? (
              <div className="relative rounded-xl border border-slate-800 overflow-hidden bg-slate-950 group">
                ...
                <div className="absolute inset-0 bg-slate-950/70 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      setValue(registerKey, '', { shouldDirty: true, shouldValidate: true })
                    }
              ...
              >
                ...
                <p className="text-[10px] text-slate-500 mt-1">
                  Soporta formatos de imagen (Máx. 5MB)
                </p>
              </div>
            )}
        ...
        {field.key !== 'fecha' && errors[registerKey] && (
          <p className="text-xs font-semibold text-rose-500 mt-1">{errors[registerKey]?.message}</p>
        )}
      </div>
    );
  };
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2 text-2xs sm:text-xs text-slate-400">
              ...
              <Link
                ...
                className="font-mono hover:text-orange-500 transition-colors truncate max-w-[80px] sm:max-w-none"
              >
                #{pedido.id}
              </Link>
              <span>/</span>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
            ...
            <Link href={`/admin/pedidos/${pedido.id}`}>
              <Button
                type="button"
                variant="outline"
                className="border-slate-800 text-slate-400 hover:text-white hover:bg-slate-900 text-[10px] sm:text-xs py-1 sm:py-1.5 px-2.5 sm:px-3 h-8"
              >
                Descartar cambios
              </Button>
            </Link>
            ...
            <div className="flex-1 overflow-y-auto p-5 pb-24 space-y-6">
              {isLoading ? (
                ...
                <form id="editor-form" onSubmit={handleSubmit(onSave)} className="space-y-6">
                  ...
                  <div className={cn('space-y-5', currentFormTab === 'info' ? 'block' : 'hidden')}>
                    <div className="space-y-4">
                      {accordionGroups.map((group) => {
                        const groupFields = infoFields.filter(
                          (f) => getAccordionGroup(f) === group.id
                        );
                        ...
                        return (
                          <div
                            key={group.id}
                            className="border border-slate-900 rounded-xl bg-[#111217]/40 overflow-hidden transition-all duration-200"
                          >
                            <button
                              type="button"
                              onClick={() =>
                            ...
                            <div
                              className={cn(
                                'p-4 space-y-4 border-t border-slate-900/60 bg-[#0e0f14]/20',
                                isOpen ? 'block' : 'hidden'
                              )}
                            >
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-slate-400">
                          ...
                          {pedido.paquete === 'esencial' && (
                            <span>
                              {' '}
                              Permite 1 foto de portada.{' '}
                              <Link
                                href={`/admin/pedidos/${pedido.id}`}
                                className="text-orange-500 underline hover:text-orange-400 font-medium"
                              >
                                Upgrade a Completa
                              </Link>{' '}
                              para galería de 3 fotos.
                            </span>
                          )}
                          {pedido.paquete === 'completa' && (
                            <span>
                              {' '}
                              Permite 1 foto de portada y 3 fotos en galería.{' '}
                              <Link
                                href={`/admin/pedidos/${pedido.id}`}
                                className="text-orange-500 underline hover:text-orange-400 font-medium"
                              >
                              ...
                              para galería de 6 fotos.
                            </span>
                          )}
                          {pedido.paquete === 'premium' && (
                            <span>
                              {' '}
                              Permite 1 foto de portada, galería de 6 fotos y fotos de
                              itinerario/historia.
                            </span>
                          )}
                        </p>
                  ...
                  {/* Tab 3: Diseño */}
                  <div
                    className={cn('space-y-4', currentFormTab === 'diseno' ? 'block' : 'hidden')}
                  >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <a
                                  ...
                                  className="text-[10px] text-orange-505 hover:underline"
                                >
                                  Ver original
                                </a>
                              </div>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-3 bg-[#0d0e12] border-b border-slate-900/80 shrink-0">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-2xs sm:text-xs uppercase tracking-wider text-slate-400 font-bold">
                  Vista previa en vivo
                </span>
              </div>

              <div
                className="flex bg-[#121318] rounded-lg p-1 border border-slate-900"
                ...
                {pedido.urlPublica && (
                  <a href={pedido.urlPublica} target="_blank" rel="noopener noreferrer">
                    <Button
                      type="button"
                      variant="outline"
                      className="border-slate-800 text-slate-400 hover:text-white hover:bg-slate-900 text-xs py-1.5 px-3 h-9"
                    >
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6 font-sans">
        <div className="max-w-md w-full border border-slate-900 bg-slate-900/40 p-6 rounded-2xl shadow-xl text-center space-y-4">
          ...
          <Button
            ...
            className="bg-[#8b5cf6] hover:bg-[#7c3aed] text-white font-semibold rounded-lg"
          >
            Recargar Página
          </Button>
        </div>
