...
import { Users, FileText, Calendar, TrendingUp, ArrowRight, PlusCircle } from 'lucide-react';
...
export default async function AdminDashboardPage() {
  ...
  visits.forEach((v) => {
    const dayString = new Date(v.fecha).toLocaleDateString('es-MX', {
      weekday: 'short',
      day: 'numeric',
    });
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 p-8 shadow-xl">
        ...
        <div className="relative space-y-4 max-w-xl">
          ...
          <p className="text-slate-400 text-base leading-relaxed">
            Aquí tienes el estado actual de tu negocio en tiempo real. Crea clientes, gestiona
            pedidos y registra pagos.
          </p>
          <div className="flex flex-wrap gap-4 pt-2">
            ...
            <Link href="/admin/pedidos">
              <Button
                variant="outline"
                className="border-slate-800 bg-slate-900/50 text-slate-300 hover:text-white hover:bg-slate-900 flex items-center gap-2"
              >
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-slate-900 bg-slate-900/20 backdrop-blur-sm shadow-md text-slate-100 hover:border-slate-800 transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-sm font-medium text-slate-400">Pedidos del Mes</CardTitle>
        ...
        <Card className="border-slate-900 bg-slate-900/20 backdrop-blur-sm shadow-md text-slate-100 hover:border-slate-800 transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-sm font-medium text-slate-400">Eventos de Hoy</CardTitle>
        ...
        <Card className="border-slate-900 bg-slate-900/20 backdrop-blur-sm shadow-md text-slate-100 hover:border-slate-800 transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-sm font-medium text-slate-400">Leads Nuevos</CardTitle>
          ...
          <CardContent className="space-y-4">
            {upcomingEvents.length === 0 ? (
              ...
              upcomingEvents.map((event) => (
                <div
                  key={event.id}
                  className="flex items-center justify-between p-3 rounded-lg border border-slate-900 bg-slate-950/40 hover:border-slate-800 transition-colors"
                >
                  <div>
                    <p className="font-semibold text-white">{event.cliente.nombre}</p>
                    <p className="text-xs text-slate-500 capitalize">
                      Evento: {event.tipoEvento} | {event.template}
                    </p>
                  </div>
                  <div className="text-right space-y-1">
                    <p className="text-sm font-semibold text-violet-400">
                      {new Intl.DateTimeFormat('es-MX', {
                        ...
                        month: 'short',
                        year: 'numeric',
                      }).format(event.fechaEvento)}
                    </p>
                    <div className="flex items-center gap-1.5 justify-end">
                      {(() => {
                        const diffTime =
                          new Date(event.fechaEvento).getTime() - startOfToday.getTime();
