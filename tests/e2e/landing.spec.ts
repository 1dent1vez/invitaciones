...
test.describe('Landing Page E2E Tests - ¡Ábreme!', () => {
  test('debería cargar la landing page con la caja cerrada y mostrar el headline principal', async ({
    page,
  }) => {
    await page.goto('/');
  ...
  test("debería abrir la caja al hacer click en 'Ábreme' y revelar las secciones subsecuentes", async ({
    page,
  }) => {
    await page.goto('/');
  ...
  test('debería mostrar los precios correctos de los paquetes leyendo de src/lib/paquetes.ts', async ({
    page,
  }) => {
    await page.goto('/');
    await page.getByTestId('btn-abreme').click();
  ...
  test('debería funcionar el enlace en el CTA final para deslizarse a la sección de contacto', async ({
    page,
  }) => {
    await page.goto('/');
    await page.getByTestId('btn-abreme').click();
  ...
  test('debería funcionar la navegación y revelarse el contenido con prefers-reduced-motion activado', async ({
    page,
  }) => {
