...
describe('PublicRSVPForm Pax Validation Test', () => {
  it('muestra error de validación cuando pax es mayor a 10', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('El límite máximo de acompañantes es 10')).toBeInTheDocument();
    });
  });
  ...
  it('rechaza mensaje de 250 caracteres', async () => {
    ...
    // Verify validation error is shown
    await waitFor(() => {
      expect(screen.getByText('El mensaje no debe superar los 200 caracteres')).toBeInTheDocument();
    });
  });
